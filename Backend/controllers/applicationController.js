...
export const postApplication = catchAsyncError(async (req, res, next) => {
    const { role } = req.user;
    if (role === "Employer") {
        return next(new ErrorHandler("Emplyer not allowed to access", 200))
    }
    if (!req.files || Object.keys(req.files).length === 0) {
        return next(new ErrorHandler("Resume file required", 400)); // Change status code to 400 for bad request
    }

    const { resume } = req.files;
    const allowedFormats = ["image/png", "image/jpeg", "image/webp"]; // Corrected MIME types for JPG and JPEG

    if (!allowedFormats.includes(resume.mimetype)) {
        return next(new ErrorHandler("Invalid file type! Resume file required in PNG, JPG, or WEBP format", 400)); // Change status code to 400 for bad request
    }
